"""
Postgres Learning Studio - Streamlit Setup UI
Step-by-step wizard with collapsible sections.
Completed steps show green and collapse; the current action step opens automatically.
"""

import time

import streamlit as st
from snowflake.snowpark.context import get_active_session

session = get_active_session()

# ============================================================
# Helper functions
# ============================================================


def call_procedure(proc_name: str, *args) -> str:
    """Call a stored procedure and return the result string."""
    try:
        return session.call(f"app_setup.{proc_name}", *args)
    except Exception as e:
        return f"ERROR: {e}"


def get_settings() -> dict:
    """Load all settings from app_config.settings."""
    try:
        rows = session.sql("SELECT key, value FROM app_config.settings").collect()
        return {row["KEY"]: row["VALUE"] for row in rows}
    except Exception:
        return {}


def upsert_setting(key: str, value: str):
    """Save a setting to app_config.settings."""
    safe_key = key.replace("'", "''")
    safe_value = value.replace("'", "''")
    session.sql(
        f"MERGE INTO app_config.settings AS t "
        f"USING (SELECT '{safe_key}' AS key, '{safe_value}' AS value) AS s ON t.key = s.key "
        f"WHEN MATCHED THEN UPDATE SET value = s.value, updated_at = CURRENT_TIMESTAMP() "
        f"WHEN NOT MATCHED THEN INSERT (key, value) VALUES (s.key, s.value)"
    ).collect()


def get_service_status() -> str:
    """Get the current service status string."""
    try:
        result = call_procedure("service_status")
        if result and "ERROR" not in str(result):
            return str(result).strip()
    except Exception:
        pass
    return "NOT_FOUND"


# ============================================================
# Step header with status badge
# ============================================================

def _step_header(step_num: int, title: str, state: str) -> str:
    """Return step header with color-coded status.

    state: 'done', 'current', 'future'
    """
    if state == "done":
        icon = "\u2705"
        color = "#0d6"
        label = "Done"
    elif state == "current":
        icon = "\u25b6\ufe0f"
        color = "#f55"
        label = "Action Required"
    else:
        icon = "\u23f3"
        color = "#888"
        label = "Pending"
    return (
        f"{icon}  **Step {step_num}: {title}**"
        f"  &nbsp; <span style='font-size:0.75rem;padding:2px 8px;border-radius:4px;"
        f"background:{color}22;color:{color};font-weight:600'>{label}</span>"
    )


# ============================================================
# Load state
# ============================================================

settings = get_settings()
svc_status = get_service_status()

# Step completion checks
step1_done = settings.get("configured", "false") == "true"

# Step 2: EAI — check via SYSTEM$GET_ALL_REFERENCES
step2_done = False
try:
    ref_result = session.sql(
        "SELECT SYSTEM$GET_ALL_REFERENCES('postgres_eai')"
    ).collect()[0][0]
    # Returns JSON array; non-empty = bound
    if ref_result and ref_result.strip() not in ("", "[]"):
        step2_done = True
except Exception:
    pass

# Step 3: Service ever created (any state except NOT_FOUND)
step3_done = svc_status != "NOT_FOUND"
step3_running = svc_status in ("READY", "RUNNING")

step4_done = False  # Gallery Operator detected

# Check Gallery Operator
app_name = "POSTGRES_LEARNING_STUDIO"
try:
    app_name = session.sql("SELECT CURRENT_DATABASE()").collect()[0][0]
except Exception:
    pass

try:
    rows = session.sql(
        "SELECT version FROM BLUE_APP_GALLERY_REGISTRY.PUBLIC.OPERATOR "
        "WHERE app_name = 'BLUE_APP_GALLERY' LIMIT 1"
    ).collect()
    step4_done = len(rows) > 0
except Exception:
    pass

# Determine current step (first incomplete)
if not step1_done:
    current_step = 1
elif not step2_done:
    current_step = 2
elif not step3_done:
    current_step = 3
elif not step4_done:
    current_step = 4
else:
    current_step = 0  # All done


def step_state(step_num: int, done: bool) -> str:
    if done:
        return "done"
    if step_num == current_step:
        return "current"
    return "future"


# ============================================================
# Page layout
# ============================================================

st.title("Postgres Learning Studio")
st.caption("Setup & Configuration")

# Overall progress
done_count = sum([step1_done, step2_done, step3_done, step4_done])
if done_count == 4:
    st.success("All setup steps are complete. Your app is ready to use.")
else:
    st.progress(done_count / 4)
    st.caption(f"Setup progress: **{done_count}/4** steps complete")

# Quick status bar
qs_cols = st.columns(4)
for i, (label, done) in enumerate([
    ("PostgreSQL", step1_done),
    ("EAI", step2_done),
    ("Service", step3_done),
    ("Gallery", step4_done),
]):
    with qs_cols[i]:
        color = "#0d6" if done else "#f55" if (i + 1) == current_step else "#888"
        st.markdown(
            f'<div style="text-align:center;font-size:0.78rem">'
            f'<span style="display:inline-block;width:10px;height:10px;border-radius:50%;'
            f'background:{color};margin-right:4px"></span>{label}</div>',
            unsafe_allow_html=True,
        )

st.divider()

# ============================================================
# Step 1: PostgreSQL Connection
# ============================================================

s1 = step_state(1, step1_done)
st.markdown(_step_header(1, "PostgreSQL Connection", s1), unsafe_allow_html=True)

with st.expander("Configure PostgreSQL connection", expanded=(s1 == "current")):
    if step1_done:
        # Show current config summary
        conn_type_label = (
            "Snowflake Postgres" if settings.get("pg_connection_type") == "snowflake_postgres"
            else "External PostgreSQL"
        )
        instance_info = ""
        if settings.get("pg_instance_name"):
            instance_info = f" / {settings.get('pg_instance_name')}"
        conn_detail = (
            f"{settings.get('pg_admin_user', '')}@"
            f"{settings.get('pg_host', '')}:{settings.get('pg_port', '5432')}"
        )
        st.markdown(
            f'<div style="font-size:0.82rem;padding:6px 10px;border-radius:6px;'
            f'background:#0d662222;border:1px solid #0d663333">'
            f'<span style="color:#0d6;font-weight:600">{conn_type_label}{instance_info}</span>'
            f'<br><span style="color:#888;font-size:0.75rem">{conn_detail}</span></div>',
            unsafe_allow_html=True,
        )
        st.caption("")

        if st.button("Reset Configuration", type="secondary", key="reset_cfg"):
            result = call_procedure("reset_config")
            st.warning(result)
            time.sleep(1)
            st.rerun()
    else:
        st.info(
            "**Configure the primary PostgreSQL connection.**\n\n"
            "This database stores benchmark results and monitoring data. "
            "It is also used as a learning/benchmark target."
        )

    # Connection type selector
    saved_conn_type = settings.get("pg_connection_type", "")
    conn_type_options = ["Snowflake Postgres", "External PostgreSQL"]
    default_idx = 0 if saved_conn_type == "snowflake_postgres" else 1 if saved_conn_type else 0
    conn_type = st.radio(
        "Connection Type",
        conn_type_options,
        index=default_idx,
        horizontal=True,
        help="Snowflake Postgres: managed instance / External: any PostgreSQL",
    )

    if conn_type == "Snowflake Postgres":
        # Discover Postgres instances
        instance_map = {}
        try:
            pg_instances = session.sql("SHOW POSTGRES INSTANCES").collect()
            for row in pg_instances:
                name = row.get("name", row.get("NAME", ""))
                host = row.get("host", row.get("HOST", ""))
                state = row.get("state", row.get("STATE", ""))
                instance_map[name] = {"host": host, "state": state}
        except Exception:
            pass

        if instance_map:
            instance_names = list(instance_map.keys())
            saved_instance = settings.get("pg_instance_name", "")
            default_instance_idx = (
                instance_names.index(saved_instance) if saved_instance in instance_names else 0
            )
            selected_instance = st.selectbox(
                "Postgres Instance",
                instance_names,
                index=default_instance_idx,
            )
            inst_host = instance_map[selected_instance]["host"]
            inst_state = instance_map[selected_instance]["state"]
            st.caption(f"Host: `{inst_host}` / State: **{inst_state}**")
        else:
            selected_instance = st.text_input(
                "Postgres Instance Name",
                value=settings.get("pg_instance_name", ""),
                help="e.g. PLEASANTER_APP_DB",
            )
            inst_host = ""
            st.caption(
                "Auto-discovery unavailable inside Native Apps. "
                "Run `SHOW POSTGRES INSTANCES;` in Snowsight to find the host."
            )

        with st.form("sf_postgres_config"):
            pg_host_sf = st.text_input(
                "Postgres Host",
                value=settings.get("pg_host", ""),
                help="Run SHOW POSTGRES INSTANCES and paste the host value",
            )
            pg_port_sf = st.text_input("Port", value="5432", disabled=True)
            pg_user_sf = st.text_input(
                "Username",
                value=settings.get("pg_admin_user", "snowflake_admin"),
            )
            pg_pass_sf = st.text_input("Password", type="password")

            submitted_sf = st.form_submit_button("Save Configuration", type="primary")

        if submitted_sf:
            if not pg_host_sf or not pg_pass_sf or not selected_instance:
                st.error("Instance Name, Host, and Password are required.")
            else:
                result = call_procedure(
                    "configure_postgres", pg_host_sf, "5432", pg_user_sf, pg_pass_sf
                )
                if "ERROR" in result:
                    st.error(result)
                else:
                    upsert_setting("pg_connection_type", "snowflake_postgres")
                    upsert_setting("pg_instance_name", selected_instance)
                    st.success(result)
                    time.sleep(1)
                    st.rerun()

    else:
        # External PostgreSQL
        with st.form("postgres_config"):
            pg_host = st.text_input(
                "Postgres Host",
                value=settings.get("pg_host", ""),
                help="FQDN of the Postgres instance",
            )
            pg_port = st.text_input("Port", value=settings.get("pg_port", "5432"))
            pg_user = st.text_input(
                "Username",
                value=settings.get("pg_admin_user", ""),
            )
            pg_pass = st.text_input("Password", type="password")

            submitted = st.form_submit_button("Save Configuration", type="primary")

        if submitted:
            if not pg_host or not pg_pass or not pg_user:
                st.error("Host, Username, and Password are required.")
            else:
                result = call_procedure("configure_postgres", pg_host, pg_port, pg_user, pg_pass)
                if "ERROR" in result:
                    st.error(result)
                else:
                    upsert_setting("pg_connection_type", "external")
                    upsert_setting("pg_instance_name", "")
                    st.success(result)
                    time.sleep(1)
                    st.rerun()

st.divider()

# ============================================================
# Step 2: EAI Approval
# ============================================================

s2 = step_state(2, step2_done)
st.markdown(_step_header(2, "External Access Integration (EAI)", s2), unsafe_allow_html=True)

with st.expander("Approve EAI for PostgreSQL connectivity", expanded=(s2 == "current")):
    if step2_done:
        st.markdown(
            '<div style="font-size:0.82rem;padding:6px 10px;border-radius:6px;'
            'background:#0d662222;border:1px solid #0d663333">'
            '<span style="color:#0d6;font-weight:600">EAI Approved</span>'
            '<br><span style="color:#888;font-size:0.75rem">'
            'The service can connect to external PostgreSQL.</span></div>',
            unsafe_allow_html=True,
        )
    elif not step1_done:
        st.caption("Complete Step 1 first.")
    else:
        st.warning(
            "**EAI approval is required** for the service to connect to PostgreSQL.\n\n"
            "Go to Snowsight and approve the External Access Integration:"
        )
        st.markdown(
            "1. Open **Snowsight** > **Data Products** > **Apps** > **Postgres Learning Studio**\n"
            "2. Click the **Security** tab\n"
            "3. Under **External Access**, click **Review** next to `postgres_eai`\n"
            "4. Click **Approve**"
        )
        st.caption("After approving, click the button below to verify.")
        if st.button("Check EAI Status", type="primary", key="check_eai"):
            st.rerun()

st.divider()

# ============================================================
# Step 3: Service Status
# ============================================================

s3 = step_state(3, step3_done)
st.markdown(_step_header(3, "Service", s3), unsafe_allow_html=True)

with st.expander("Service status and Web UI", expanded=(s3 == "current")):
    if not step1_done or not step2_done:
        st.caption("Complete previous steps first.")
    elif step3_running:
        # Service running — show Web UI link
        st.markdown(
            '<div style="font-size:0.82rem;padding:6px 10px;border-radius:6px;'
            'background:#0d662222;border:1px solid #0d663333">'
            f'<span style="color:#0d6;font-weight:600">Service: {svc_status}</span></div>',
            unsafe_allow_html=True,
        )
        web_url = call_procedure("get_web_ui_url")
        if web_url and "ERROR" not in str(web_url) and web_url.strip():
            st.markdown(
                f'<a href="https://{web_url}" target="_blank" '
                f'style="display:inline-block;margin-top:8px;padding:10px 20px;'
                f'background:#0d6efd;color:white;border-radius:8px;'
                f'text-decoration:none;font-weight:bold;">'
                f'Open Postgres Learning Studio</a>',
                unsafe_allow_html=True,
            )
            st.caption(f"URL: https://{web_url}")
        else:
            st.info("Endpoint URL is being provisioned. Refresh in a moment.")
    elif step3_done:
        # Service exists but not running — managed by Gallery
        st.markdown(
            '<div style="font-size:0.82rem;padding:6px 10px;border-radius:6px;'
            'background:#0d662222;border:1px solid #0d663333">'
            f'<span style="color:#0d6;font-weight:600">Service created</span>'
            f'<br><span style="color:#888;font-size:0.75rem">'
            f'Current status: {svc_status} — '
            f'Start/stop is managed by Gallery Operator.</span></div>',
            unsafe_allow_html=True,
        )
    else:
        # Service never created — first-time setup
        st.info(
            "**Create the service for the first time.**\n\n"
            "This is a one-time action. After creation, Gallery Operator "
            "will manage start/stop."
        )
        if st.button("Create Service", type="primary", key="create_svc_first"):
            result = call_procedure("start_service")
            if "ERROR" in str(result):
                st.error(result)
            else:
                st.success(result)
                time.sleep(2)
                st.rerun()

    # Troubleshooting (always available when service has been created)
    if step1_done and step3_done:
        st.markdown("---")
        st.caption(
            "If the service is stuck or unreachable, you can recreate it here."
        )
        t_cols = st.columns(2)
        with t_cols[0]:
            if st.button("Recreate Service", key="recreate_svc"):
                call_procedure("drop_service")
                result = call_procedure("start_service")
                st.info(result)
                time.sleep(2)
                st.rerun()
        with t_cols[1]:
            if st.button("Fetch Logs", key="fetch_logs"):
                logs = call_procedure("service_logs", 100)
                if "ERROR" in str(logs):
                    st.error(logs)
                else:
                    st.code(logs, language="text")

st.divider()

# ============================================================
# Step 4: Gallery Operator Integration
# ============================================================

s4 = step_state(4, step4_done)
st.markdown(_step_header(4, "Gallery Operator Integration", s4), unsafe_allow_html=True)

with st.expander("Connect to Gallery Operator", expanded=(s4 == "current")):
    if step4_done:
        st.markdown(
            '<div style="font-size:0.82rem;padding:6px 10px;border-radius:6px;'
            'background:#0d662222;border:1px solid #0d663333">'
            '<span style="color:#0d6;font-weight:600">Gallery Operator Connected</span></div>',
            unsafe_allow_html=True,
        )
    else:
        st.info(
            "Run the following GRANTs in a **Snowsight SQL Worksheet** as **ACCOUNTADMIN** "
            "to connect this app with Gallery Operator."
        )

    # Build GRANT SQL with placeholders
    pool_name = settings.get("compute_pool", "")
    pg_instance = settings.get("pg_instance_name", "")
    pool_display = pool_name if pool_name else "<COMPUTE_POOL_NAME>"
    pg_display = pg_instance if pg_instance else "<POSTGRES_INSTANCE_NAME>"

    grant_sql = (
        f"-- Run in Snowsight Worksheet as ACCOUNTADMIN\n\n"
        f"-- 1. Registry access (Gallery Operator detection)\n"
        f"GRANT USAGE ON DATABASE BLUE_APP_GALLERY_REGISTRY\n"
        f"    TO APPLICATION {app_name};\n"
        f"GRANT USAGE ON SCHEMA BLUE_APP_GALLERY_REGISTRY.PUBLIC\n"
        f"    TO APPLICATION {app_name};\n"
        f"GRANT SELECT ON TABLE BLUE_APP_GALLERY_REGISTRY.PUBLIC.OPERATOR\n"
        f"    TO APPLICATION {app_name};\n"
        f"\n"
        f"-- 2. App role (allows Gallery Operator to manage this app)\n"
        f"GRANT APPLICATION ROLE {app_name}.app_admin\n"
        f"    TO APPLICATION BLUE_APP_GALLERY;\n"
        f"\n"
        f"-- 3. Compute Pool (start/stop control)\n"
        f"GRANT OPERATE ON COMPUTE POOL {pool_display}\n"
        f"    TO APPLICATION BLUE_APP_GALLERY;\n"
        f"GRANT MONITOR ON COMPUTE POOL {pool_display}\n"
        f"    TO APPLICATION BLUE_APP_GALLERY;\n"
    )

    if settings.get("pg_connection_type") == "snowflake_postgres":
        grant_sql += (
            f"\n"
            f"-- 4. Postgres Instance (resumed/suspended by Operator)\n"
            f"GRANT OPERATE ON POSTGRES INSTANCE {pg_display}\n"
            f"    TO APPLICATION BLUE_APP_GALLERY;\n"
        )

    st.code(grant_sql, language="sql")

    if not pool_name or (settings.get("pg_connection_type") == "snowflake_postgres" and not pg_instance):
        placeholders = []
        if not pool_name:
            placeholders.append("`<COMPUTE_POOL_NAME>`")
        if settings.get("pg_connection_type") == "snowflake_postgres" and not pg_instance:
            placeholders.append("`<POSTGRES_INSTANCE_NAME>`")
        st.caption(
            f"Placeholders {', '.join(placeholders)} will be replaced "
            "when resources are configured."
        )

    if not step4_done:
        if st.button("Check Gallery Operator", type="primary", key="check_gallery"):
            st.rerun()

st.divider()

# ============================================================
# Advanced Settings (always at bottom, collapsed)
# ============================================================

st.markdown("**Advanced Settings**")

with st.expander("Service Resource Limits", expanded=False):
    st.caption(
        "Default settings are sufficient for most use cases. "
        "Changes take effect on next service start."
    )
    with st.form("resource_config"):
        col1, col2 = st.columns(2)
        with col1:
            cpu_request = st.text_input("CPU Request", value=settings.get("cpu_request", "0.5"))
            memory_request = st.text_input("Memory Request", value=settings.get("memory_request", "1Gi"))
        with col2:
            cpu_limit = st.text_input("CPU Limit", value=settings.get("cpu_limit", "2"))
            memory_limit = st.text_input("Memory Limit", value=settings.get("memory_limit", "4Gi"))

        if st.form_submit_button("Save Resource Settings"):
            upsert_setting("cpu_request", cpu_request)
            upsert_setting("cpu_limit", cpu_limit)
            upsert_setting("memory_request", memory_request)
            upsert_setting("memory_limit", memory_limit)
            st.success("Resource settings saved. Restart the service to apply.")

# PostgreSQL Extensions (Snowflake Postgres only)
saved_instance_name = settings.get("pg_instance_name", "")
is_sf_postgres = settings.get("pg_connection_type") == "snowflake_postgres"

if is_sf_postgres and saved_instance_name:
    with st.expander("PostgreSQL Extensions", expanded=False):
        st.caption(f"Target Instance: **{saved_instance_name}**")
        st.info(
            "To load extension libraries, use `ALTER POSTGRES INSTANCE` "
            "(PostgreSQL's internal `ALTER SYSTEM` is restricted)."
        )
        st.warning(
            "Currently only **`auto_explain`** is supported in `session_preload_libraries`."
        )

        if st.button("Check Instance Settings", key="check_pg_settings"):
            try:
                rows = session.sql(
                    f"DESCRIBE POSTGRES INSTANCE {saved_instance_name}"
                ).collect()
                st.dataframe(rows)
            except Exception as e:
                st.error(f"Failed: {e}")

        AVAILABLE_PRELOAD_LIBS = ["auto_explain"]
        preload_libs = st.multiselect(
            "Libraries to add to session_preload_libraries",
            AVAILABLE_PRELOAD_LIBS,
            default=[],
        )

        if st.button("Apply session_preload_libraries", type="primary", key="apply_preload"):
            if not preload_libs:
                st.warning("Select at least one library.")
            else:
                lib_value = ",".join(preload_libs)
                alter_sql = (
                    f'ALTER POSTGRES INSTANCE {saved_instance_name} '
                    f'SET POSTGRES_SETTINGS = \'{{"postgres:session_preload_libraries": "{lib_value}"}}\''
                )
                st.code(alter_sql, language="sql")
                try:
                    session.sql(alter_sql).collect()
                    st.success(f"`session_preload_libraries = '{lib_value}'` set.")
                except Exception as e:
                    st.error(f"Failed: {e}")

elif not is_sf_postgres and step1_done:
    with st.expander("PostgreSQL Extensions", expanded=False):
        st.caption(
            "Using external PostgreSQL. Configure extensions via "
            "`shared_preload_libraries` on the PostgreSQL side."
        )

with st.expander("Additional Target Connections", expanded=False):
    st.info(
        "Register additional benchmark/learning targets from the **Connections** page "
        "in the Web UI.\n\n"
        "**Note:** When adding a new host, the EAI network rule must include that host."
    )
